...
import { editProjectMilesrone } from "../../../api/projects";
...
const addMilestone = async () => {
  try {
    await editProjectMilesrone(`${route.params.project}`, `${props.id}`, {
...
watch(
  () => [props.id, props.comment, props.date_end, props.date_start, props.name],
  () => {
    createdMilestone.value.comment = props.comment;
    createdMilestone.value.date = dayjs(`${props.date_start}`);
    createdMilestone.value.date_end = dayjs(`${props.date_end}`);
    createdMilestone.value.milestone = props.name;
  },
  {
    immediate: true,
  }
